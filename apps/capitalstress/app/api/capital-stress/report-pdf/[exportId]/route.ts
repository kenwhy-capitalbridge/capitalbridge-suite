...
export async function GET(request: NextRequest, ctx: { params: Promise<{ exportId: string }> }) {
  ...
  reportPdfApiTimingLog("capital-stress", "request_start", apiT0, { export_id_prefix: id.slice(0, 8) });
  ...
  reportPdfApiTimingLog("capital-stress", "session_and_report_exports_ok", apiT0);
  ...
  reportPdfApiTimingLog("capital-stress", "before_renderPdf", apiT0, {
    budget_ms: budgetMs,
  ...
  try {
    ...
    reportPdfApiTimingLog("capital-stress", "renderPdf_error", apiT0, { message: msg.slice(0, 200) });
    return NextResponse.json({ error: msg }, { status: 500 });
  }

  reportPdfApiTimingLog("capital-stress", "after_renderPdf", apiT0, { pdf_bytes: pdf.length });
