...
export function MembershipSessionCheck({ initialSafeMode = false }: MembershipSessionCheckProps) {
  ...
  /** Strip `cb_all_set` from URL after access flows without showing a toast. */
  useEffect(() => {
    let cancelled = false;
  ...
  const shellSyncingClass =
    "fixed inset-x-0 top-0 z-[100] border-b-2 border-amber-600/35 bg-amber-100 px-3 py-3 text-center text-sm text-cb-green shadow-md sm:px-4 sm:py-3.5";

  const shellVerifyClass =
    "fixed inset-x-0 top-0 z-[100] border-b-2 border-sky-700/35 bg-sky-50 px-3 py-3 text-center text-sm text-cb-green shadow-md sm:px-4 sm:py-3.5";
  ...
  return (
    <>
      {banner?.variant === "syncing" ? (
        <div className={shellSyncingClass} role="status">
          <p className="text-base font-semibold tracking-tight">{COPY_SYNCING_ACCESS}</p>
        </div>
      ) : null}

      {banner?.variant === "verify" ? (
        <div className={shellVerifyClass} role="status">
