...
const MODEL_TYPE: ModelType = "income-engineering";
...
export function AdvisoryShell({
  ...
  const handleSave = useCallback(async () => {
    ...
    if ("error" in out) {
      console.warn("[income-engineering] saveReport error:", out.error);
      setSaveStatus("error");
      return;
    }
    setSaveStatus("ok");
  ...
  const handleRestore = useCallback((_inputs: Record<string, unknown>) => {
    // TODO: rehydrate app state when app has form state
  }, []);
