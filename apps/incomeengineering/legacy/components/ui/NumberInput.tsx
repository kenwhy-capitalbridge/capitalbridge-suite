...
/** Format a raw digit string (e.g. "12345" or "12345.67") with commas as user types. */
function formatRawWithCommas(raw: string, allowDecimals: boolean): string {
  ...
  if (allowDecimals && stripped.includes('.')) {
    ...
    const formattedInt = intPart === '' ? '' : new Intl.NumberFormat('en-US').format(parseInt(intPart || '0', 10));
...
/** Return position in formatted string where exactly n digits appear before. */
...
/**
 * Number input that displays the value with thousand-separator commas as the user types.
 */
export const NumberInput: React.FC<NumberInputProps> = ({
  ...
  const digitsBeforeCursorRef = useRef<number>(0);
  ...
  const displayValue = focused
    ? (localRaw === '' && value === 0 ? '' : formattedFromRaw || '0')
    : (allowDecimals ? formatNumberWithDecimals(value, decimalPlaces) : formatWithCommas(value));
