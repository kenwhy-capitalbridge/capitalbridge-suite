 ...
 *
 * Set `REPORT_PDF_CAPTURE_SECRET` in production (or rely on `SUPABASE_SERVICE_ROLE_KEY` as fallback secret).
 */
...
export function signPdfCaptureToken(args: {
  exportId: string;
  userId: string;
  ttlSec?: number;
}): string | null {
...
export function verifyPdfCaptureToken(token: string, expectedExportId: string): PdfCapturePayload | null {
  const secret = captureSecret();
  if (!secret) return null;
